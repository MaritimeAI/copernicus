...
class DataHub:
    ...
    def search(self, params: Dict[str, Any], area=None) -> List[Snapshot]:
        ...
        if not isinstance(params, dict):
            print(f"No params (see Config.search)!")
            return []
        ...
        if 'filenames' in params and isinstance(params['filenames'], list):
    ...
    def _download(self, source: str, chunk_size=524288, index=None) -> str:
        ...
        try:
            ...
            if not osp.exists(self.config.output):
            ...
            target = osp.join(self.config.output, filename)
            if osp.exists(target) and os.stat(target).st_size == filesize:
                if self.config.verbose:
...
class OpenSearchAPI:
    ...
    def compose_query_params(params: Dict[str, Any]) -> str:
        return ' OR '.join(f"filename:{str(v)}" for v in params['filenames']) \
               ...
               ' AND '.join(f"{k}:{str(v)}" for k, v in params.items() if k != 'filenames')
